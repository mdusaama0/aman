                ...
                return Column(
                  children: [
                    GestureDetector(
                      onTap: () {},
                      child: ListTile(
                          title: getlinklist[index].user2Email == 'Test' ||
                                  getlinklist[index].user2Email == null
                              ? Image.asset(
                                  'assets/images/realone.png',
                                  width: 60,
                                  height: 60,
                                )
                              : Image.network(getlinklist[index].user2Email!,
                                  width: 60, height: 60),
                          leading: Text(getlinklist[index].user1Email!),
                          trailing: RaisedButton(
                            onPressed: () {
                              SimpleAlertBox(
                                  context: context,
                                  title: getlinklist[index].user1Email,
                                  infoMessage: getlinklist[index].lastMessage,
                                  icon: Icons.info);
                            },
                            child: Text('Details'),
                          )),
    ...
    if (ore.getString('counter') == null) {
      ...
      if (response.statusCode == 401) {
        ...
        for (var u in responseJson) {
          chatsdialogmodel post = chatsdialogmodel(u['id'].toString(),
              u['countryName'].toString(), u['imagePath'], u['description']);
